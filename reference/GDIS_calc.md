...
This function calculates GDIS parameters. Input is based on LDSC
...
can also add, this will allow GDIS to check its calculated values
...
``` r
GDIS_calc(triangle.input.list, log_fun = message, webversion = FALSE)
```
...
additional input: plot_3D: this tells GDIS that you want to calculate a
...
some optional input here as well, will allow GDIS to check its
...
triangle2.h2_sub2.con plot_2D.2D: this tells GDIS to run in the subtype
